// out-of-hydrant.js (polygon/line layer only, no pins)
...
const url = PATH_PREFIX + 'out_of_hydrant_district.geojson?v=' + Date.now();
fetch(url).then(r=>r.json()).then(raw=>{
  const data = normalizeGeoJSON(raw);
  const layer = L.geoJSON(data, { style:()=>({ color:'#b91c1c', weight:2, fillOpacity:0.08 }), onEachFeature: defaultPopup }).addTo(map);
  const b = L.latLngBounds([]);
  layer.eachLayer(l=>{ if(l.getBounds) b.extend(l.getBounds()); else if(l.getLatLng) b.extend(l.getLatLng()); });
