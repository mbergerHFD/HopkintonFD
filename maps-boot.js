...
(function () {
  ...
  function ensureLeafletCSS() {
    const has = [...document.styleSheets].some((ss) =>
      ss.href && /leaflet(\.min)?\.css/i.test(ss.href)
    );
    if (has) return;
  ...
  // Try loading Leaflet JS from multiple CDNs if not present
  function loadLeafletFromCDNs() {
    return new Promise((resolve, reject) => {
      ...
      function tryNext() {
        ...
        script.async = false; // we want predictable execution order
        script.onload = () => resolve(window.L);
        script.onerror = () => setTimeout(tryNext, 50);
  ...
  // If Leaflet is not present in time, auto-load it; otherwise just resolve
    ...
    return new Promise((resolve, reject) => {
      ...
      const tick = setInterval(() => {
        if (window.L && typeof window.L.map === "function") {
          ...
          clearInterval(tick);
          // Timed out waiting — proactively load from CDNs
          loadLeafletFromCDNs()
            .then((L) => resolve(L))
            .catch((e) => reject(e));
        }
      ...
      // also resolve on full load (covers iOS bfcache)
