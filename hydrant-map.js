// hydrant-map.js — clean single-guard build (mobile-safe)

(async function () {
  try {
    await HFD.whenDOMReady();
    await HFD.whenLeafletReady(8000); // waits for window.L on iOS too
  } catch (e) {
    ...
    const coreTable = `
      <table style="width:100%; border-collapse: collapse; font-size:0.92rem;">
        ${coreRows
          .map(
            ([k, v]) =>
              `<tr><td style="padding:4px 6px; color:#374151;"><strong>${esc(k)}</strong></td><td style="padding:4px 6px;">${esc(
                v
              )}</td></tr>`
          )
          .join("")}
  ...
  async function fetchAny(paths) {
    for (let path of paths) {
      try {
        ...
        if (!r.ok) {
          console.warn("[hydrant-map] Not found:", absolute, r.status);
          continue;
        }
        ...
        try {
          ...
          console.warn("[hydrant-map] Parse failed; sanitizing:", absolute, e.message);
  ...
  function init() {
    ...
    // Mobile sizing fix
    const fixSizeSoon = (d = 0) => setTimeout(() => map.invalidateSize(true), d);
    ...
    const icon = L.divIcon({
      ...
      html:
        '<svg viewBox="0 0 24 24" width="18" height="18" fill="#ef4444" stroke="#991b1b" stroke-width="1.5"><circle cx="12" cy="12" r="6"/></svg>',
    });

    fetchAny(hydrantSources())
      .then((geojson) => {
        ...
        if (feats.length === 0) {
          console.warn(
            "[hydrant-map] No features to render. Falling back to no-filter circleMarkers for debug."
          );
          feats = all;
          const layer = L.geoJSON(
            { type: "FeatureCollection", features: feats },
            {
              pointToLayer: (feature, latlng) =>
                L.circleMarker(latlng, { radius: 5, color: "#ef4444", weight: 2, fillOpacity: 0.7 }),
    ...
    // Bind search (global binder if present)
    if (typeof window.HFD_bindToolbarSearch === "function") {
      try {
        window.HFD_bindToolbarSearch(map, { zoom: 16, defaultCity: "Hopkinton" });
      } catch {}
    }
  }
