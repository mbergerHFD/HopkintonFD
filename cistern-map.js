// cistern-map.js (pure cisterns only; excludes dry hydrants & landing zones)
...
fetch(url).then(r=>r.json()).then(raw=>{
  ...
  const layer = L.geoJSON(fc, { pointToLayer:(f,latlng)=>L.marker(latlng,{icon:cisternIcon(),riseOnHover:true}), onEachFeature: defaultPopup }).addTo(map);
