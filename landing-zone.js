// landing-zone.js (only landing zones extracted from cisterns file)
...
fetch(url).then(r=>r.json()).then(raw=>{
  ...
  const feats = (data.features||[]).filter(f=>f.geometry && f.geometry.type==='Point' && isLandingZone(f.properties));
  const fc = {type:'FeatureCollection', features:feats};
  const layer = L.geoJSON(fc, { pointToLayer:(f,latlng)=>L.marker(latlng,{icon:helicopterIcon(),riseOnHover:true}), onEachFeature: defaultPopup }).addTo(map);
